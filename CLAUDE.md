...
- **San Andreas 1.0 EU**: accepted even though the SDK only claims 1.0 US. The two images are address-identical below `0x741000`; `0x741000-0x7C0FFF` is displaced by `+0x50` and the rest of `.text` by `+0x40`. Every global the mod reads and every function it calls lives in the identical region. The one exception was `Events::initGameEvent`, whose hook site `0x748CFB` falls in the `+0x50` range, so **the SA target does not register that event at all** — `OnGameInit()` runs from the first `OnGameProcess()` tick instead, guarded by `m_initialised`. Do not reintroduce `initGameEvent`, `reInitGameEvent` or any other address at or above `0x741000` on the SA path without re-checking it, or EU breaks again. `tools/check_addresses.py` and `tools/compare_exe.py` reproduce the analysis against a pair of exes; `tools/identify_exe.py` reports which build an exe is.
